...
describe("Stripe Live Integration Tests", () => {
  ...
  const config = {
    apiKey: apiKey || "sk_test_mock_key_for_ci",
  ...
  itIfKey("should fetch real products from the Stripe dashboard", async () => {
    ...
    if (products.length > 0) {
      ...
      console.log(
        "⚠️ No active products found. Check if products are 'Active' in Dashboard."
      );
    }
  });
   ...
   */
  itIfKey(
    "should verify the presence of the specific TEST_PRICE_ID",
    async () => {
      ...
      const foundProduct = products.find(
        (p: any) => p.priceId === targetPriceId
      );

      if (foundProduct) {
        console.log(
          `✅ Success: Found target product "${foundProduct.name}" with Price ID: ${targetPriceId}`
        );
      }
   ...
   */
  itIfKey(
    "should generate a Customer Portal URL for a sample customer",
    async () => {
      // 1. Fetch a sample customer from your Stripe account to test the portal
      const customers = await (service as any).stripe.customers.list({
        limit: 1,
      });

      if (customers.data.length > 0) {
        const customerId = customers.data[0].id;
        const portal = await service.createPortalSession(
          customerId,
        ...
        console.log(
          "⏭️ Skipping Portal test: No customers found in Stripe account."
        );
      }
    }
  );
